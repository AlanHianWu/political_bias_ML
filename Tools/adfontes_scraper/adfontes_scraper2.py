...
# Used for interacting with CSV files
import csv
# Used for time delay functionality
import time
# For searching functionality
import re
# For parsing JSON filetypes
import json
#import pandas as pd
...
def scrape_sources():
    ...
    #print(bias_list)
    _out = [''] * 9999
    # Use RegEx to find instances of 'url'
...
# open 'output2.html' and search for elements matching ID
    ...
    with open('output2.html', 'r') as f:
        ...
        with open("output.csv", "w") as csvfile:
            ...
            for bias in bias_list:
                ...
                reliability_value = float(bias['y'])
                #if bias_value < 0:
                #    bias_value = bias_value+804.88
                #else:
                    #bias_value = bias_value-432.43
                #    bias_value = bias_value-627.18
                filewriter.writerow([bias['href'].split('/')[-1].split('.png')[0], bias['href'], bias_value, reliability_value])
        print("output.csv written")
